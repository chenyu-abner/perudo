...
class ComputerPlayer(AI):
    ...
    def choose_action(self, q_table, current_bet):
        ...
        P_current=self.prob(self.game.total_dice-len(self.dice), current_bet.quantity - self.count_dice(current_bet.value), current_bet.value)
        ...
        if P_current < 0.1:
            ...
            if np.random.uniform() < epsilon:
                ...
                for k in range(1,self.game.total_dice+2-current_bet.quantity):
    ...
    def Qtable(self,current_bet,bet):
        ...
        if self.choose_action(self.table, current_bet) != DUDO:
            ...
            Max = 0
            for k in range(0,self.game.total_dice+1):
                for l in range(0,7):
            ...
            q_target = self.reward(current_bet) * 10 * (self.game.play_no - len(self.game.players))
            self.rewards += q_target
            # if correct dudo, reward = 10
